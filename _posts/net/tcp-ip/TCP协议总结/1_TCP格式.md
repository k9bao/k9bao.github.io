...
上图中，保留的(6位)状态位，目前只有三位保留，后三位分别用于 NS CWDR NS
...
&emsp;&emsp;对于紧急指针是指向紧急数据的最后一个字节还是指向紧急数据最后一个字节的下一个字节的争论。最初的 TCP 规范给出了两种解释，但Host Requirements RFC确定指向最后一个字节是正确的。然而，问题在于大多数的实现（包括源自伯克利的实现）继续使用错误的解释。
...
比如Data是8，则2^8=256,比如windows size大小为513，则实际窗口大小为513*256
